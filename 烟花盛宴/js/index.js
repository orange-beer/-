...
function randomShellName() {
	return Math.random() < 0.5 ? 'Crysanthemum 水晶花' : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0 ];
}
...
const fastShellBlacklist = ['Falling Leaves 落叶', 'Floral 花朵', 'Willow 柳树'];
function randomFastShell() {
	const isRandom = shellNameSelector() === 'Random 随机';
...
const shellTypes = {
	...
	'Crackle 裂纹': crackleShell,
	'Crossette 克罗塞特': crossetteShell,
	'Crysanthemum 水晶花': crysanthemumShell,
	'Falling Leaves 落叶': fallingLeavesShell,
	'Floral 花朵': floralShell,
	'Ghost 幽灵': ghostShell,
	'Horse Tail 马尾': horsetailShell,
	'Palm 棕榈树': palmShell,
	'Ring 戒指': ringShell,
	'Strobe 斯特罗布': strobeShell,
...
function init() {
	...
	setOptionsForSelect(appNodes.quality, [
		{ label: 'Low 低', value: QUALITY_LOW },
		{ label: 'Normal 正常', value: QUALITY_NORMAL },
		{ label: 'High 高', value: QUALITY_HIGH }
	]);
	
	setOptionsForSelect(appNodes.skyLighting, [
		{ label: 'None 无', value: SKY_LIGHT_NONE },
		{ label: 'Dim 昏暗', value: SKY_LIGHT_DIM },
		{ label: 'Normal 正常', value: SKY_LIGHT_NORMAL }
	]);
